...
const AllAnimePage: React.FC = () => {
  ...
  const title = searchParams.get('title') || 'All Anime';
  const endpoint = searchParams.get('endpoint');

  useEffect(() => {
    // Reset page to 1 whenever the category (endpoint) changes.
    setPage(1);
  }, [endpoint]);
  ...
  const renderContent = () => {
    ...
    if (animeList.length === 0) {
      return <div className="flex justify-center items-center h-[60vh]"><p className="text-gray-400">No anime found for this category.</p></div>;
    }
    return (
      <>
